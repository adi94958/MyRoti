...
const KelolaDataKoordinator = () => {
  const [searchText, setSearchText] = useState('')
  const [dataKoordinator, setDataKoordinator] = useState([])
  const navigate = useNavigate()
  const [currentPage, setCurrentPage] = useState(1)
  const [itemsPerPage, setItemsPerPage] = useState(10)
  const itemsPerPageOptions = [10, 25, 50, dataKoordinator.length] // Jumlah data per halaman
  ...
  function handleEdit(data) {
    ...
    navigate('/admin/koordinator/update')
  }
  ...
  const filteredData = dataKoordinator.filter((akun) => {
  ...
  const endIndex =
    itemsPerPage === dataKoordinator.length ? dataKoordinator.length : startIndex + itemsPerPage
  ...
  return (
    <div>
      <CRow>
        <CCol>
          <CCard>
            <CCardHeader>Data Koordinator</CCardHeader>
            <CCardBody>
              <CForm className="mb-3">
                <CRow>
                  ...
                  <CCol md={3} xs={4} className="text-start">
                    <Link to="/admin/koordinator/tambah">
                    ...
                    >
                      {itemsPerPageOptions.map((option) => (
                        <option key={option} value={option}>
                          {option === dataKoordinator.length ? 'All' : option}
                        </option>
                      ))}
                    </CFormSelect>
                  </CCol>
...
export default KelolaDataKoordinator
